...
impl Client {
    ...
    pub fn connect(
        ...
        if self.address_to_connection.contains_key(&remote_address) {
            return Err(ClientError::AlreadyConnectedToAddress {
    ...
    pub fn send(&mut self, packet: OutgoingPacket, connection: Connection) -> Result<u64, Error> {
        match self.get_connection_state(connection) {
            ...
            Some(ConnectionState::Connecting) => Err(ClientError::ConnectionStillConnecting.into()),
            Some(ConnectionState::Disconnected) => Err(ClientError::ConnectionDisconnected.into()),
            None => Err(ClientError::ConnectionNotFound.into()),
        }
    }
    ...
    fn handle_message(
        ...
        let secret = self
            .secrets
            .get(connection)
            .expect("Secret for connection not found");

        if let Some(incoming) = packet.verify(secret) {
            let state = self
                .states
                .get(connection)
                .expect("State for connection not found")
                .clone();
    ...
    fn send_connection_message(&mut self, connection: Connection) -> Result<(), Error> {
        ...
        let connection_token = self
            .connection_tokens
            .get(connection)
            .expect("No connection token for connection");
